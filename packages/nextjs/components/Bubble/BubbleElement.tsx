...
const BubbleWrapper: React.FC<BubbleWrapperProps> = ({ children }) => {
...
const BubbleElement: React.FC<BubbleElementProps> = ({ children, options: userOptions, className, style }) => {
  ...
  const interpolate = (actualMin: number, actualMax: number, val: number, targetMin: number, targetMax: number) => {
  ...
  const getBubbleSize = (row: number, col: number) => {
    ...
    if (Math.abs(dx) <= options.xRadius && Math.abs(dy) <= options.yRadius) {
      ...
      isInCornerRegion =
        Math.abs(dy) > options.yRadius - options.cornerRadius && Math.abs(dx) > options.xRadius - options.cornerRadius;
      if (isInCornerRegion) {
        const distToInnerCorner = Math.sqrt(
          Math.pow(Math.abs(dy) - options.yRadius + options.cornerRadius, 2) +
            Math.pow(Math.abs(dx) - options.xRadius + options.cornerRadius, 2),
  ...
  return (
    ...
    >
      <div className={styles.container}>
        <div className={styles.scrollable} ref={scrollable} onScroll={e => handleScroll(e)}>
          <div
          ...
          >
            {rows.map((row, i) => {
              return (
                ...
                >
                  {row.map((comp, j) => {
                    const { bubbleSize, translateX, translateY, distance } = getBubbleSize(i, j);
                    ...
                    const style = {
                      ...
                      transform: `translateX(${round(translateX, 2)}px) translateY(${round(
                        translateY,
                        2,
                      )}px) scale(${round(bubbleSize, 2)})`,
                    };
                    return (
                      <div key={j} className={styles.bubbleContainer} style={style}>
